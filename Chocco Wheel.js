...
//Find Us
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const findUsLink = document.querySelector('.Locate');
        const popup = document.getElementById('popup-box');
        const closeBtn = document.querySelector('.close-popup');

        if (findUsLink) {
            findUsLink.addEventListener('click', (e) => {
                e.preventDefault();
                if (popup) {
...
const initMenuApp = (jsonFileName = 'Chocco Wheel.json') => {
                ...
                itemElement.innerHTML = `
                    ...
                    <div class="gallery-item-info">
                        <div class="gi_content">
                            <h3>${product.name}</h3>
                            <span class="category">${product.category || 'Medicine'}</span>  
                        </div>  
    ...
    function openLightbox(images, name, index) {
        if (!lightbox) return;
        currentGalleryImages = images;
        currentIdx = index;
        lightboxImg.src = currentGalleryImages[currentIdx];
    ...
    lightbox?.querySelector('.lightbox-next')?.addEventListener('click', () => {
        currentIdx = (currentIdx + 1) % currentGalleryImages.length;
        lightboxImg.src = currentGalleryImages[currentIdx];
    });

    lightbox?.querySelector('.lightbox-prev')?.addEventListener('click', () => {
        currentIdx = (currentIdx - 1 + currentGalleryImages.length) % currentGalleryImages.length;
        lightboxImg.src = currentGalleryImages[currentIdx];
    });
...
document.getElementById('final-send-btn').addEventListener('click', () => {
    ...
    if (branch === "mannarkkad") {
        whatsappNumber = "918921137074";
    } 
    else if (branch === "kalladikode") {
        whatsappNumber = "919447938750";
    } 
    else if (branch === "mundur") {
        whatsappNumber = "919074292491";
    }
